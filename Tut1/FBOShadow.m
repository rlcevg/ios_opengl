...
//        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        ...
        NSLog(@"5: %x", glGetError());
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        NSLog(@"6: %x", glGetError());
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_HALF_FLOAT_OES, NULL);
//        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F_EXT, width, height, 0, GL_RGBA, GL_FLOAT, NULL);  // NOTE: ES3+ only
        NSLog(@"7: %x", glGetError());
...
//        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F_EXT, width >> BLUR_COEF, height >> BLUR_COEF, 0, GL_RGBA, GL_FLOAT, 0);  // NOTE: ES3+ only
